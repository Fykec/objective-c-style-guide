转载自[NYTimes-Objective-C-Style-Guide-ZH](https://github.com/VincentSit/NYTimes-Objective-C-Style-Guide-ZH)
# 译者的话
...
这里是苹果提供的一些风格指南，如果有什么东西没在这里提到，那它可能被下面其中一个更详细的所涵盖：   
...
* 一个缩进使用 4 个空格，永远不要使用制表符（tab）缩进。请确保在 Xcode 中设置此偏好。
...
```
一些苹果的 API 在成功的情况下会写一些垃圾值给错误参数（如果非空），所以针对错误可以造成虚假结果（以及接下来的崩溃）。
...
通常应该避免一大块注释，代码就应该尽量作为自身的文档，只需要隔几行写几句说明。这并不适用于用来生成文档的注释。   
...
常量首选内联字符串字面量或数字，因为常用的变量可以轻易重用并且可以快速改变而不需要找到然后再替换。常量应该声明为 `static` 常量而不是 `#define` ，除非非常明确地要当做宏来使用。   
...
图片名称命名应始终保持组织与开发稳健性。应该使用驼峰命名法来描述它们的意图，其次是要自定义的类或属性的无前缀名字（如果有要自定义的话），最后进一步说明颜色 和/或 展示位置，和它们最终的状态。   
...
自从 `nil` 可以作为 `NO` 使用之后，已经没有必要在条件中和它对比。永远不要拿某个东西直接和 `YES` 比较，因为 `YES` 被定义为 1，而 `BOOL` 可以多达 8 位。
...
为了避免文件杂乱，物理文件应该保持和 Xcode 项目文件同步。Xcode 创建的任何组都必须在文件系统有相应的映射。为了更清晰，代码不仅应该按照类型进行分组，也应该根据特点进行分组。


如果可以的话，尽可能一直打开 target Build Settings "Treat Warnings as Errors" 以及一些[额外的警告][Xcode-project_1]。如果你需要忽略指定的警告,使用 [Clang 的编译特性][Xcode-project_2] 。
